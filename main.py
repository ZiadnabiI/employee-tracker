...
# One-time password migration endpoint (remove after running!)
@app.get("/migrate-passwords-{secret}")
def migrate_passwords_endpoint(secret: str):
    """
    One-time endpoint to migrate SHA-256 passwords to bcrypt.
    Access via: /migrate-passwords-YOUR_SECRET_KEY
    DELETE THIS ENDPOINT AFTER RUNNING!
    """
    import secrets as sec
    import string
    
    # Security check - must know a secret from env
    expected_secret = os.getenv("MIGRATION_SECRET", "run-migration-2024")
    ...
    try:
        # Migrate supervisors
        supervisors = db.query(Supervisor).filter(Supervisor.password_hash.isnot(None)).all()
        
        for sup in supervisors:
            if sup.password_hash and sup.password_hash.startswith('$2b$'):
                results.append({"email": sup.email, "status": "already_bcrypt"})
                continue
            
            # Generate temp password
            chars = string.ascii_letters + string.digits
            chars = chars.replace('0', '').replace('O', '').replace('l', '').replace('1', '')
            temp_pass = ''.join(sec.choice(chars) for _ in range(10))
            
            sup.password_hash = hash_password(temp_pass)
            results.append({
                "type": "supervisor",
                "email": sup.email,
                "name": sup.name,
        ...
        # Migrate employees with passwords
        employees = db.query(Employee).filter(Employee.password_hash.isnot(None)).all()
        
        for emp in employees:
            if emp.password_hash and emp.password_hash.startswith('$2b$'):
                results.append({"name": emp.name, "status": "already_bcrypt"})
                continue
            ...
            emp.password_hash = hash_password(temp_pass)
            results.append({
                "type": "employee", 
                "email": emp.email,
                "name": emp.name,
        ...
        return {
            ...
            "message": "SAVE THESE PASSWORDS! Users need them to log in.",
